...
class sv_shm {
   ...
   public:
    ...
    void* map(int size = BUFSIZE);  // size esta por compatibilidad con las
                                    // posix. Se usa BUFSIZE
    string getClave() { return clave; }
    friend ostream& operator<<(ostream& os, sv_shm shm);
};
...
ostream& operator<<(ostream& os, sv_shm shm) {
    return os << "Memoria compartida con ID " << shm.getShm() << " y clave 0x"
              << hex << shm.getKtClave() << dec << " en (" << shm.getClave()
              << ")";
}
#endif
